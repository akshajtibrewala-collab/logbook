...
// Exercises 013's up() function directly against a hand-built database that already has aircraft/flights
...
test('seeds one aircraft_rates row per non-simulator aircraft, effective from the earliest flight date', async () => {
  ...
  await run("INSERT INTO flights (date, total_time) VALUES ('2025-03-10', 1)");
  await run("INSERT INTO flights (date, total_time) VALUES ('2025-01-05', 1)");
  ...
  assert.equal(rates[0].effective_date, '2025-01-05'); // earliest flight date, not insertion order

  const instructor = await get('SELECT * FROM instructor_rates');
  assert.equal(instructor.effective_date, '2025-01-05');
